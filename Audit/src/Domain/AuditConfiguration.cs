    ...
    where TUserKey : IEquatable<TUserKey>, IComparable<TUserKey>
{
    ...
    /// </summary>
    public static AuditConfiguration<TUser, TUserKey> Create()
        => new();
    ...
    public bool IsValidUserKey(object? userKey)
        => userKey is TUserKey;
    ...
    public TUser? CastUser(object? user)
        => user as TUser;
    ...
    public TUserKey? CastUserKey(object? userKey)
        => userKey is TUserKey key ? key : default;
    ...
    public bool IsDefaultUserKey(TUserKey? userKey)
        => EqualityComparer<TUserKey>.Default.Equals(userKey, DefaultUserKey);
}
...
/// </summary>
internal sealed class AuditConfigurationAdapter<TUser, TUserKey> : IAuditConfiguration
    where TUser : IdentityUser<TUserKey>
    where TUserKey : IEquatable<TUserKey>, IComparable<TUserKey>
{
    private readonly AuditConfiguration<TUser, TUserKey> _config;

    public AuditConfigurationAdapter(AuditConfiguration<TUser, TUserKey> config)
    {
        _config = config;
    }

    public Type UserType => _config.UserType;
    public Type UserKeyType => _config.UserKeyType;

    public bool IsValidUser(object? user) => _config.IsValidUser(user);
    public bool IsValidUserKey(object? userKey) => _config.IsValidUserKey(userKey);
}