...
public class AuditableConfigurationTests
{
   [Fact]
   public void Configure_Should_SetPrimaryKey()
   {
      ...
      var entityBuilder = builder.Entity<AuditableEntity<int, IdentityUser<int>, int>>();
      var configuration = new AuditableConfiguration<int, IdentityUser<int>, int>();

      // Act
      configuration.Configure(entityBuilder);

      // Assert
      var entityType = builder.Model.FindEntityType(typeof(AuditableEntity<int, IdentityUser<int>, int>));
   ...
   {
      // Arrange
      var auditTrail = new AuditableEntity<Guid, IdentityUser<int>, int>
                       {
                          Id         = Guid.NewGuid(),
   ...
   public void Configure_WithGuidKey_ShouldConfigureGuidIdProperty()
   {
      // Arrange
      var builder       = new ModelBuilder();
      var entityBuilder = builder.Entity<AuditableEntity<Guid, IdentityUser<int>, int>>();
      var configuration = new AuditableConfiguration<Guid, IdentityUser<int>, int>();

      // Act
      configuration.Configure(entityBuilder);

      // Assert
      var entityType = builder.Model.FindEntityType(typeof(AuditableEntity<Guid, IdentityUser<int>, int>));
      Assert.NotNull(entityType);
