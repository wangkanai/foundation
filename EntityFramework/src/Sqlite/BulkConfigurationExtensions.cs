...
public static class BulkConfigurationExtensions
{
   ...
   /// <summary> Configures transaction settings for SQLite bulk operations with large data volumes. </summary>
   ...
   public static EntityTypeBuilder<T> EnableSqliteBulkTransactions<T>(
      this EntityTypeBuilder<T> builder, IsolationLevel isolationLevel = IsolationLevel.ReadCommitted, int commandTimeout = 300, bool enableDeferred = true
   )
      where T : class
   {
      builder.ThrowIfNull();

      commandTimeout.ThrowIfOutOfRange(int.MinValue, 0);
      if (commandTimeout < 0)
   ...
   public static EntityTypeBuilder<T> OptimizeForSqliteBulkUpdates<T>(
      this EntityTypeBuilder<T> builder, bool enableRowLevelLocking = true, bool optimizeIndexes = true, int batchSize = 500
   )
      where T : class
   {
      builder.ThrowIfNull();
